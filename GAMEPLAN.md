...
| 6 | **The superyachts are anchored beyond the fog wall, and the "fake water" at the horizon is the fog sheet itself.** Owner (2026-07-30): "those massive yachts… just make them visible and then fix the horizon — the horizon has some fake water and then the real water horizon; the fake horizon fucks it up; in this game [the horizon] should only be real things." Diagnosed by reading, not yet fixed: the roadstead march (`offshore()`) demands `clearanceFor(draft)` ⇒ \|shore\| ≥ ~101 for the 156 m hull and marches OUTWARD from the harbour seed across the −33.5-capped bay ring and the country-belt ridge, so the qualifying anchorage lands kilometres offshore — while city fog tops at 1000–1400 (quality tiers) and the on-foot camera far is `max(1400, fog+180)`: the ships are fully fog-silhouetted or past the far plane from every coast view. They exist, they float, nobody can ever see them. The horizon band the owner reads as "fake water" is the fog wall standing between the real sea's own fade and sky.js's dome band (everything below the dome horizon is pinned to fog colour by design). Fix sketch: a visibility-first anchorage pass (same depth acceptance test, nearest deep water to the CITY coast, prefer < ~800 m) + a `WATER_REAL_HORIZON` treatment letting the sea read to its own horizon instead of the fog sheet — the airborne branch already proves 4200 m fog over one ocean mesh + one country mesh is affordable. | yachts.js:1153-1156, 1212-1242; quality.js:65-73; mode.js:361-386; sky.js:110-209 | **UNOWNED — next wave** (owner parked it here mid-wave after three 529-overload builder attempts: "leave it, add it to the list") |
