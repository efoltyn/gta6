...
A rule that governs every mechanic in Parts V and VI: **every person in the world is a stored individual, and simulation events transition people — they never delete-and-respawn them.** The codebase already half-commits to this ("finite factions": a permanently wipeable ~40-officer police force, police.js:65-90; non-respawning gang rosters; the 600-entry NPC ledger with permanent death via `dropSid`). The plan completes it: the ledger becomes the **population registry** (backed by SQLite, Part VI.7) covering everyone — cops, soldiers, officials, billionaires, shopkeepers, citizens — each with identity, wallet, employer, home, relationships, and faction history. Regime changes, war, unemployment, and bankruptcies *re-assign* people (cop → former cop with his gun; conscripted worker → soldier → veteran; laid-off clerk → gang recruit pool), and every transition is visible and consequential. Spawning is presentation only: bodies materialize near the player *from* registry entries, never invented at random.
