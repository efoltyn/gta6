...
async function boot(seed, quiet) {
  ...
  /* CBZ_PAGE picks the HTML entry point. The default is index.html (the hub:
     it builds the gang city, then the island modes sit inside its
     coordinates). The disaster island has its OWN page — disaster.html, the
     App Store build, generated by tools/build-disaster-page.mjs — and every
     question about the survival world is better asked there: no city
     materials to wade through, no skyline standing on the island, and a boot
     that is seconds rather than a minute. Before this the tool could only
     ever answer for index.html, so a volcano question had to boot the whole
     city to get to the island. */
  const entry = process.env.CBZ_PAGE ? String(process.env.CBZ_PAGE).replace(/^\/+/, "") : "";
  const target = `${origin}${entry ? "/" + entry : ""}?seed=${seed}${extra}`;
