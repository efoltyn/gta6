/*
  ba/lib/engine.mjs — deterministic visual comparison runner.
  ...
  Raw CDP over Node's global WebSocket, no npm dependencies, on purpose: a
  receipt machine that needs a 300MB install is a receipt machine nobody runs.

  Ported out of one game repo, so the four things that were welded to that
  repo now come from ba.config.mjs instead — the preset directory, the
  baseline URL, how the local build is served, and where reports land. The
  in-page hook is `window.__ba`, with the old `window.__cbzVisualCompare`
  still honored so existing presets keep working unmodified. Environment
  overrides are BA_* with CBZ_* accepted as a fallback for the same reason.

  Called as a library:
    import { run } from "./engine.mjs";
    await run(["--preset", "beach-shores", "--before", url], ctx);
*/
...
import { resolvePresetPath } from "./config.mjs";
import { startForConfig } from "./serve.mjs";

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));

/* BA_FOO, or CBZ_FOO for anyone still carrying the old name in a CI file. */
const envVar = (suffix) => process.env[`BA_${suffix}`] || process.env[`CBZ_${suffix}`] || "";

/* THE IN-PAGE HOOK. A deterministic preset installs a small object on the
   page so the runner can drive the frozen simulation: `render()` pushes one
   composited frame, `advance(sec)` steps the sim between film-strip shots,
   `metrics()` hands back numbers sampled over exactly those frames. New name,
   old name still answered — read it once into a local in every snippet. */
const HOOK = "(window.__ba || window.__cbzVisualCompare)";
...
async function printReportFresh(htmlPath, pdfPath, chromeBin, cwd) {
  const printerDir = await mkdtemp(path.join(tmpdir(), "ba-visual-print-"));
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      ], { cwd, timeout: 120000, maxBuffer: 1024 * 1024 }, (err) => err ? reject(err) : resolve());
    });
...
/* DEVICE FRAMES ------------------------------------------------------------
   ...
   touch, user agent, screen orientation — because a build branches on all of
   them (a `body.touch` class alone can decide whether phone controls exist).
...
/* WHICH BROWSER. Explicit env wins, then the project's own config, then the
   two locations that have actually been found on real machines. Nothing here
   downloads a browser: a tool with no dependencies does not get to install
   400MB behind your back. */
export function resolveBrowser(config) {
  return envVar("CHROME")
...
export function helpText(config) {
  const outDir = config && config.out ? config.out : "<config.out>";
  return `ba <preset> [flags] — before/after report\n\n` +
    `  --preset NAME|FILE   visual recipe (name resolves in ${config?.presets || "<config.presets>"})\n` +
    ...
    `  --out DIR            report directory (default: ${outDir}/<preset>-<stamp>)\n` +
    `  --reuse-before DIR   reuse a completed run's before shots + stage metadata\n` +
    `                       (fast visual iteration without reopening the baseline)\n` +
    ...
    `  --json               print machine-readable results after the run\n` +
    `  --no-summary         do not print the measurements table\n`;
}

/**
 * Run one comparison.
 * @param {string[]} argv engine flags (--preset ... --before ...)
 * @param {{projectRoot: string, config: object, stdout?: NodeJS.WritableStream}} ctx
 * @returns {Promise<{reportDir: string, htmlPath: string|null, pdfPath: string|null,
 *                    presetName: string, preset: object|null, only: string|null}>}
 */
export async function run(argv, ctx = {}) {
  const config = ctx.config || {};
  const ROOT = ctx.projectRoot || config.projectRoot || process.cwd();
  ...
  if (args.help) {
    say(helpText(config));
    return { reportDir: null, htmlPath: null, pdfPath: null, presetName: null, preset: null, only: null };
  }

  if (!args.preset || args.preset === true) {
    throw new Error("--preset NAME|FILE is required");
  }
  const presetName = String(args.preset);
  const presetPath = resolvePresetPath(presetName, { projectRoot: ROOT, presets: config.presets || ROOT });
  ...
  const beforeUrlRaw = String(args.before || envVar("VISUAL_BEFORE") || reuseMetadata?.before?.final
    || preset.defaultBefore || config.baseline || "");
  if (!beforeUrlRaw) {
    throw new Error("--before URL is required (or preset.defaultBefore, or `baseline` in ba.config.mjs; " +
      "the value \"local\" runs a same-checkout flag-A/B — see --before-params)");
  }
  ...
  const defaultOut = path.join(config.out || path.join(ROOT, "artifacts", "ba"),
    `${preset.id || safeName(presetName)}-${stamp}`);
  const outputDir = path.resolve(ROOT, String(args.out || defaultOut));
  ...
  const chromeBin = resolveBrowser(config);
  const debugPort = 10400 + Math.floor(Math.random() * 500);
  const portHint = 8700 + Math.floor(Math.random() * 500);

  // Printing an existing report needs no server, DevTools connection, or WebGL
  // capture browser. Return before allocating any of those resources so the
  // fresh standalone renderer is genuinely fresh.
  if (args["print-only"]) {
    const htmlPath = path.join(outputDir, "report.html");
    const pdfPath = path.join(outputDir, "before-after.pdf");
    await printReportFresh(htmlPath, pdfPath, chromeBin, ROOT);
    say(`\nPDF reprinted: ${pdfPath}\n`);
    if (!args["no-open"] && process.platform === "darwin") {
      const opener = spawn("open", [pdfPath], { detached: true, stdio: "ignore" });
      opener.unref();
    }
    return { reportDir: outputDir, htmlPath, pdfPath, presetName, preset, only: null };
  }
  ...
  /* "--before local" (or preset.defaultBefore = "local"): the SAME checkout
     ...
     a BEHAVIOR change — a deployed build differs by every commit since deploy,
     a flag flip differs by exactly the change under test. The tool used to make
     this impossible: the local port is chosen at run time, after --before had
     to already be a concrete URL. */
  const beforeIsLocal = beforeUrlRaw === "local";
  const startsLocalServer = (!args.after && !envVar("VISUAL_AFTER")) || beforeIsLocal;

  const profileDir = await mkdtemp(path.join(tmpdir(), "ba-visual-compare-"));
  const children = [];
  ...
  let pdfPath = null;
  try {
    // The local build, served by ba's own static server or by whatever the
    // project declared in `serve`. Started BEFORE the URLs are decided, so
    // both sides can be pointed at the port it actually got.
    if (startsLocalServer) localServer = await startForConfig(config, { port: portHint });
    const localUrl = localServer ? localServer.url : `http://127.0.0.1:${portHint}/`;
    const afterUrl = String(args.after || envVar("VISUAL_AFTER") || localUrl);
    ...
    /* THE BEFORE-SIDE IS ON THE INTERNET, and some environments only reach it
       ...
       and BA_CHROME_ARGS is there for anything else the host needs to pass.

       FIELD NOTES from a TLS-intercepting sandbox, because the failure is silent
       and the diagnosis took a netlog: a MITM proxy re-signs every site, and
       Chromium trusts only its NSS store — the CA env vars that satisfy
       curl/node do nothing for it. Import the proxy bundle first:
       ...
       TLS 1.3 hello — BA_CHROME_ARGS="--ssl-version-max=tls1.2" negotiates
    ...
    const envProxy = envVar("CHROME_PROXY") || process.env.HTTPS_PROXY || process.env.https_proxy || "";
    const proxyArgs = envProxy ? [
      `--proxy-server=${envProxy}`,
      `--proxy-bypass-list=${envVar("CHROME_PROXY_BYPASS") || "127.0.0.1;localhost;[::1]"}`,
    ] : [];
    const extraChromeArgs = envVar("CHROME_ARGS").split(/\s+/).filter(Boolean);
    ...
    chrome.on("error", (err) => {
      say(`\nchrome failed to start (${chromeBin}): ${err.message}\n` +
        `Set BA_CHROME or \`browser\` in ba.config.mjs to a chrome/chromium binary.\n`);
    });
    ...
    /* INJECTED PAGE HELPERS. A preset that declares `inject: ["staging"]` gets
       lib/staging-page.js evaluated into the page after navigation and before
       the first stage runs — the shared staging vocabulary (waiting on real
       conditions, installing the __ba hook) that every preset otherwise had to
       paste into its own stage function. Evaluated per navigation because each
       one is a fresh JS context. A missing file is not fatal: `inject` is a
       request for a helper, not a hard dependency, and a preset that does not
       need one must not care whether it shipped. */
    const injectNames = Array.isArray(preset.inject) ? preset.inject.filter(Boolean).map(String) : [];
    const injectSources = new Map();
    for (const name of injectNames) {
      const file = fileURLToPath(new URL(`./${safeName(name)}-page.js`, import.meta.url));
      if (!existsSync(file)) {
        say(`[inject] ${name}: no ${path.basename(file)} in this build, continuing without it\n`);
        continue;
      }
      injectSources.set(name, await readFile(file, "utf8"));
    }
    async function injectHelpers() {
      for (const [name, source] of injectSources) {
        const message = await send("Runtime.evaluate", {
          expression: `${source}\n//# sourceURL=ba-inject-${safeName(name)}.js`,
          returnByValue: false,
          awaitPromise: true,
        }, 30000);
        if (message.exceptionDetails) {
          const detail = message.exceptionDetails.exception?.description || message.exceptionDetails.text;
          throw new Error(`inject "${name}" failed: ${detail || "unknown error"}`);
        }
      }
    }
    ...
    function parseParamString(value) {
      ...
      for (const pair of String(value || "").split("&")) {
        ...
        if (eq < 0) result[pair] = "1";
        else result[pair.slice(0, eq)] = pair.slice(eq + 1);
      }
      return result;
    }

    /* WHICH PAGE OF THE BUILD. --before/--after name a BUILD ROOT and the
       runner used to navigate straight to it, so only the site's index page
       could ever be compared: a project whose interesting screen lives at
       games/battle.html had no way to say "the same two builds, but that
       page". `preset.page` is that one word, resolved against whichever root
       each side was given, so a deployed baseline and a local checkout stay
       directly comparable. Absolute values are honoured for the rare preset
       that needs a different origin. */
    ...
    function cacheBusted(url, side) {
      ...
      // flips one cfg_* flag OFF so the before side runs the pre-change code
      // path from the same checkout; afterParams exists for symmetry. The CLI
    ...
    // build: a touch class, the quality tier and the control layout are often
    // decided once at startup, so flipping touch on an already-loaded page would
    // photograph a phone-sized window still wearing its desktop controls.
    ...
    async function reuseBeforeResult() {
      ...
      say(`[before] reused ${captures.length} matched shots from ${reuseBeforeDir}\n`);
      return {
    ...
    async function captureSide(side, sourceUrl, referenceResult = null) {
      ...
      for (let index = 0; index < subjects.length; index++) {
        ...
        say(`[${side}] ${frame.id} ${index + 1}/${subjects.length} ${subject.label || subject.id}\n`);
        ...
        try {
          ...
          say(`[${side}] ${subject.id} FAILED (kept going): ${err.message}\n`);
        ...
        /* FILM STRIP — motion photographed as stills. A still cannot show "he
           ...
           page's __ba.advance(stepSec) hook steps ITS OWN frozen simulation
           between captures, so both sides photograph the identical simulated
           seconds. After the strip, the optional metrics() hook merges numbers
           the preset sampled over those exact photographed frames — the metric
           and the pictures describe the same moment, by construction. */
        let stripFiles = null;
        ...
        if (stripSpec && Number(stripSpec.frames) > 1 && stageResult && stageResult.ok === true) {
          ...
          try {
            const sampled = await evaluate(
              `(() => { const H = ${HOOK}; return H && H.metrics ? H.metrics() : null; })()`);
    ...
    function reportHtml(before, after) {
      ...
      const pairNote = preset.pairNote || "Same recipe · seed · camera · light · viewport";
      const method = preset.method || "Both sides are photographed from a real browser build. The runner holds subject, random seed, viewport, camera framing, backdrop, and lighting constant so the source change is the variable.";
    ...
    say(`Frames: ${frames.map((frame) => `${frame.id} ${frame.width}x${frame.height}@${frame.dsf}x`).join(", ")}\n`);
    ...
    // The capture browser is carrying two complete simulated worlds and is the
