...
(function () {
  ...
  function render() {
    ...
    const touch = isTouch();
    const rowDefs = touch && d.touchRows ? d.touchRows : d.rows;
    let rows = "";
  ...
  C.declare("boat", {
    ...
    rows: [
      ...
      ["A / D", "Steer"],
      ["G", "Step back from the helm"],
      ["F", "Step off — under way, that means over the side"],
    ],
    touchRows: [
      ["STICK", "Throttle and steer"],
      ["ASTERN", "Crash-stop / reverse thrust"],
      ["SEAT", "Step back from the helm"],
      ["EXIT / JUMP OUT", "Step off — under way, that means over the side"],
    ],
  });
