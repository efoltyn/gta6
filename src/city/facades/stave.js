...
(function () {
  ...
  CBZ.registerFacade("stave", {
    ...
    build: function (ctx, F) {
      ...
      const tim = P.base, timD = F.shade(tim, 0.66), timL = P.light;
      ...
      const eo = cl(u * 0.055, 0.34, 0.85);          // the eave flare overhang
      ...
      for (const f of F.faces(ctx)) {
        const n = Math.max(6, Math.round(f.span / cl(u * 0.036, 0.30, 0.56)));
        const st = f.span / n;
        for (let i = 0; i < n; i++) {
          const t = -f.span / 2 + (i + 0.5) * st, j = ctx.hash(0x5700 + f.s * 41 + i);
          F.rib(ctx, f, t, pod.top, rTop, st * (0.60 + j * 0.26), 0.09 + j * 0.08,
            F.shade(tim, 0.84 + j * 0.32));
        }
        ...
        for (let i = 2; i < n - 1; i += Math.max(4, n >> 2)) {
          ...
          if (!F.clearsDoor(ctx, f, t, st * 2)) continue;
          F.box(ctx, f, t, rTop - cl(FH * 0.55, 1.0, 1.9), st * 1.5, st * 1.5, 0.22, P.glass, 0.03);
        }
      }
      ...
      const tier = function (y0, hw, hd, h, tw, td, k, vg) {
        ...
        ctx.dbox(0, y0 + 0.17, 0, (hw + fl) * 2, 0.28, (hd + fl) * 2, timL);     // the eave board
        ctx.dbox(0, y0 + 0.40, 0, (hw + fl * 0.5) * 2, 0.18, (hd + fl * 0.5) * 2, shD);
        for (let i = 0; i < n; i++) {
          const v = (i + 1) / n, j = 0.92 + ctx.hash(0x57a0 + k * 37 + i) * 0.18;
          const cw = Math.max(0.20, hw * (1 - (1 - tw) * v)), cd = Math.max(0.12, hd * (1 - (1 - td) * v));
          ctx.dbox(0, y0 + h * (i + 0.5) / n, 0, cw * 2, h / n + 0.05, cd * 2,
      ...
      const alongX = ctx.w >= ctx.d;
      const hw0 = ctx.w / 2 + eo * 0.5, hd0 = ctx.d / 2 + eo * 0.5;
      ...
      for (let k = 1; k <= NT; k++) {
        ...
        for (const ax of [0, 1])                                             // its belfry openings
          ctx.dbox(0, ty + dh * 0.56, 0, tw * (ax ? 1.7 : 2.06), dh * 0.46, tw * (ax ? 2.06 : 1.7), P.glass);
      ...
      for (let k = 0; k < 3; k++)                    // the mast over the crossing
        ctx.dbox(0, ty + 0.16 + k * cl(u * 0.024, 0.18, 0.36), 0, tw * (1.5 - k * 0.35),
          cl(u * 0.024, 0.18, 0.36), tw * (1.5 - k * 0.35), k % 2 ? timL : timD);

      // ---- E. THE DRAGON HEADS ------------------------------------
      const dragon = function (x, y, z, dx, dz, s) {
        for (let i = 0; i < 6; i++) {               // the neck, which must BEND
          const v = i / 5, r = s * (0.15 + v * 2.15), ry = s * (1.55 * v - 0.58 * v * v);
          ctx.dbox(x + dx * r, y + ry, z + dz * r,
            s * (0.34 - v * 0.11) + Math.abs(dx) * s * 0.22, s * (0.42 - v * 0.11),
            s * (0.34 - v * 0.11) + Math.abs(dz) * s * 0.22, i % 2 ? tim : timD);
        }
        ...
        for (let k = 0; k < 3; k++)                 // the crest down the nape
          ctx.dbox(hx - dx * s * k * 0.36, hy + s * (0.46 + k * 0.11), hz - dz * s * k * 0.36,
            s * 0.17, s * (0.36 - k * 0.07), s * 0.17, timL);
      };
      ...
      for (const sg of [-1, 1])                               // and the gable apexes
        dragon(alongX ? sg * rdg : 0, rTop + nav + 0.10, alongX ? 0 : sg * rdg,
          alongX ? sg : 0, alongX ? 0 : sg, ds * 0.92);
    },
  });
})();
