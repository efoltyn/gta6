/* ============================================================
   ...
   Each unit: { id, lot, building, floorY, door, tier, rent, occupant }.
   floorY is the unit's real slab height (ped._unit.floorY feeds sleep /
   elevator arrival in peds.js + aigoals.js). The TOP floor (home.floorY,
   the player-buyable penthouse tier) is NEVER rented out.
...
(function () {
    ...
    for (let k = 0; k < storeys; k++) {
      ...
      for (let u = 0; u < per; u++) {
        ...
        out.push({
          ...
          floorY: fy,
          door,
          tier: 0,                 // MICRO tier — the affordable floor
          rent,
          occupant: null,
        });
      }
  ...
  function build() {
    const city = CBZ.city;
    const homeLots = city && city.homeLots;
    const units = [];
  ...
  function leaseFor(ped) {
    ...
    for (let i = 0; i < SCAN; i++) {
      const u = free[i];
      if (!u || u.occupant) continue;            // (compacted lazily below)
    ...
    if (!pick) {
      ...
      for (let i = 0; i < c.units.length; i++) { if (!c.units[i].occupant) { pick = c.units[i]; break; } }
  ...
  function claim(ped, unit) {
    ...
    // release any prior lease first (re-home cleanly).
    if (ped._unit && ped._unit !== unit && ped._unit.occupant === ped) ped._unit.occupant = null;
    unit.occupant = ped;
    ped._unit = unit;
    ped._digs = unit.lot;
  ...
  function homeOf(ped) {
    ...
    const c = ensure();
    if (!c) return null;
    // still-live lease wins (the persistent bond — same door every day).
    if (ped && ped._unit && ped._unit.occupant === ped &&
    ...
    // fallback before we loaded) → adopt it, leasing a unit on it if we can so
    // floorY/occupancy line up.
    if (ped && ped._digs && c.byLot.has(ped._digs)) {
      const lus = c.byLot.get(ped._digs);
      let u = null;
      for (let i = 0; i < lus.length; i++) { if (!lus[i].occupant) { u = lus[i]; break; } }
  ...
  function release(ped) {
    ...
    const u = ped._unit;
    if (u) {
      if (u.occupant === ped) u.occupant = null;
      const hm = u.lot && u.lot.building && u.lot.building.home;
      if (hm && hm._tenants) hm._tenants = Math.max(0, hm._tenants - 1);
      // return it to the cheap-first free pool so it can be re-leased.
      if (cache && cache.free && cache.free.indexOf(u) < 0) {
  ...
  function markOccupancy() {
    ...
    for (let i = 0; i < c.units.length; i++) {
      ...
      const occ = u.occupant && !u.occupant.dead;
      // "home at night" ≈ occupant present this run + it's dark. We only flip a
      // pre-existing emissive scalar; never create/dispose anything.
  ...
  CBZ.cityHousing = {
    ...
    occupied: function () { const u = ensure().units; let n = 0; for (let i = 0; i < u.length; i++) if (u[i].occupant && !u[i].occupant.dead) n++; return n; },
    vacant: function () { const u = ensure().units; let n = 0; for (let i = 0; i < u.length; i++) if (!u[i].occupant) n++; return n; },
  };
})();
